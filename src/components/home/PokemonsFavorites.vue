  ...
  <div class="row justify-center">
    <div class="fit row wrap justify-start items-start content-start">
      <div v-for="(f, index) in _favorites" v-bind:key="index" class="q-ma-xl">         
        ...
        <q-icon name="delete_outline" size="xs" v-on:click="deleteItem(f.id)" class="pointer" color="primary" v-bind:title="$t('remove_favorite')"/>
        <p class="text-primary text-center">{{ f.name }}</p>
